...
## Melt Kernel v4.5（2026.02.07）

又是一次没啥值得说的常规更新。

原因嘛，1月初不幸染上甲流，在家躺尸，痛失3天元旦假期；痊愈之后沉迷于《幻兽帕鲁》无法自拔，每天下班后都会投入四五个小时进去。。。

听我狡辩，这不是我摆烂的主要原因，主要原因总结起来就三个字：没活了。

自2023年07月27日Melt Kernel发布v2.0版本开始到现在已经经过了两年半的时间（要素察觉？），在我看来无论是源代码还是高度自定义的安装程序，Melt Kernel都已趋近于完美。

但是，这两个月我也不是啥也没干，回顾一下，还是做了点微小的工作。

一个是在hamjin大佬的帮助下给内核添加了optprobe特性，可以进一步减小kprobes的开销。

另一个是对安装程序的改进，弃用了bsdiff4，改用更加先进的 [HDiffPatch](https://github.com/sisong/HDiffPatch)，生成差分补丁的速度、打补丁的速度、生成差分补丁时的RAM占用、打补丁时的RAM占用，各项指标都要显著优于bsdiff4。

再一个是改进了与NeotericOS的兼容性，修复了在锁屏界面拉不出来解锁图案的bug。

最后，考虑到KernelSU的源代码已基本稳定，因此把susfs加回来了。从频道的投票结果来看，使用susfs的用户仍然不少。
...
## Melt Kernel v5.1（2026.08.20）

v5.1加了个比较有意思的功能： [为疼讯系手游伪装内核版本](https://github.com/Pzqqt/android_kernel_xiaomi_marble/compare/3c1f1a2973a0...c5fc7bce8b9c) ，这个功能是在arter97大佬的补丁上做改造得来的（原始补丁的作用是为GMS伪装内核版本以通过Play Integrity检查）。由于疼讯系手游检测设备环境的方式并不明确，我自己也不玩这些手游，因此虽然这个功能确实能伪装内核版本，但我也不清楚是否管用。另外需要注意的是，susfs本身也有伪装内核版本的功能，为避免冲突，如果你在安装Melt Kernel或Bouquet Kernel时选择了需要susfs支持，那么是没有这个功能的。

然后是一个比较重要的修复：修复了开源的相机驱动导致的内存泄漏问题。这个问题还是比较严重的，只要相机一打开就会一直泄漏。

另外是针对Bouquet Kernel的修复：修复了指纹传感器手势不生效的问题。在我把crDroid升级到20260613版本之后，发现维护者添加了一个解锁设备后双击指纹传感器触发特定操作的功能，感觉挺有意思，但自己试了下并没有效果。我很清楚这是内核的bug，但整个七八月份我都太忙了一直忘了修，一直拖到最近突然想起来了才把它修了。

然后是其他一些无关紧要的优化和修复，我就不详细说了。

最后，在内测群群友的疯狂安利下，我也是花了点时间给Bouquet Kernel写好了workflow配置文件，以后Bouquet Kernel也有CI build了。非常好GitHub Actions，使我SSD寿命延长。![滑稽](/images/huaji.png)

既然Bouquet Kernel有CI build了，那么Melt Kernel呢？

在Bouquet Kernel诞生之前，我也有考虑过给Melt Kernel整上CI build，但最终还是放弃。原因有多个方面：

第一个原因是Melt Kernel的打包方式非常复杂，每次出包我得编译至少4次，第一次是编译vendor modules，第二次是编译普通的内核镜像，第三次是编译带KernelSU的内核镜像，第四次是编译带KernelSU+susfs的内核镜像，有时还要根据情况编译额外的vendor modules（每次编译都需要20分钟左右，这也是我很难支持其他非官方版KernelSU的原因之一）。并且，Melt Kernel包含了从小米HyperOS提取的一些预编译的vendor modules，以及预编译的dtb和dtbo，要在workflow中完成这些提取、替换、打补丁操作，难度非常之大。

第二个原因，可以说是我的一点私心。Melt Kernel和Bouquet Kernel独特的安装方式在目前的第三方内核中可以说算是独树一帜了。不同于其他的内核，Melt Kernel不止会替换内核镜像，还会替换dtb、dtbo、以及所有的vendor modules（也请不要以为Melt Kernel做了什么多余的事，我提到的这些东西在GKI诞生之前本身就是内核内的东西，只不过Google通过GKI把它们剥离了出来而已）。为了达到这个目标，无论是高度定制化的AnyKernel3，还是数千行的编译和打包脚本，我都花了无数的心思在里边，最终形成了我独特的优势，我并不想让这种技术烂大街。但对于真正热爱学习的内核开发者，看看这篇上万字的博文吧，我该说的都已经说清楚了，我能分享的都已经分享了，小白非得想着走捷径那我也没办法。

第三个原因，在于我对如今内核开发圈子的一些负面看法。AI的普及，让许多新人开发者高估了自己的能力，他们用AI写代码，用AI移植补丁，用AI写commits，用AI写changelog，仿佛AI无所不能。但是，AI很难对整个内核源代码有全面的认知，这会导致其生成的代码天生带有缺陷，结果很可能是引入新的bug降低整体的代码质量，或者让开发者陷入自我满足的幻觉之中。AI在大佬手中是得心应手的工具（参考weishu使用Coding Agent为KernelSU开发了metamodule功能），但AI并不能填补小白和大佬的差距。正如我在频道里所发表的观点：如果你不擅长做某一件事，而AI替你做了，那么以后你照样是不擅长这件事。AI终究是一种工具，提升自己的知识水平更加重要，“学习”这件事情是没有捷径的。

综上，尽管我花了五六个小时还是把Melt Kernel的workflow配置文件写出来了，但为了避免我的成果被小白和AI拿去霍霍，因此CI build仅作技术验证，不予公开。

## Melt Kernel v5.2（2026.??.??）
