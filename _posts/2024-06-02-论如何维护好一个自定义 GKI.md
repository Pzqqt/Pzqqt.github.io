...
{% include pure-img-responsive.html url="/images/e45_p1.png" a_class="pure-u-md-2-3" img_style="background: white;" %}
...
2. GKI和GKI模块之间不具有ABI稳定性，这意味着受保护的（也就是签过名的）GKI模块在能够使用KMI符号的同时还可以使用非KMI符号（但绝对不允许供应商模块使用）。
3. GKI模块和硬件没有太大的关联性。
4. 受保护的（也就是签过名的）GKI模块可以导出符号用于KMI。

{% include pure-img-responsive.html url="/images/e45_p3.png" a_class="pure-u-md-2-3" img_style="background: white;" %}
...
那么，我们可以这样理解：GKI模块相当于是从以前的GKI中剥离出的可精简的一部分。从 [Google发布的GKI release builds](https://source.android.com/docs/core/architecture/kernel/gki-release-builds) 中我们也可以看到，GKI模块基本上都是和蓝牙、网络相关的，因为一些嵌入式设备可能不需要这些。

还有一个疑问：为了强制要求供应商模块只能使用稳定的KMI符号，GKI在编译过程中默认会修剪掉所有非KMI符号，但GKI模块又能使用非KMI符号，这是否冲突？并不冲突，GKI模块使用的非KMI符号会被刻意保留，不会被修剪掉。
...
> 注：从`android14-6.1`开始，Google所打造的编译环境和编译脚本真的是越来越复杂了，我个人是强烈建议不要再像4.x时代那样编译内核了（即：拉内核源码然后直接make），老老实实按Google的套路来吧。
...
在我看来，GKI是未来，从此以后，从传统内核镜像中剥离出来的内核模块一定会和供应商的那堆不开源的elf文件一样成为厂商和供应商的秘密，厂商和供应商的开源积极性将会越来越低。内核开发者只需专注于GKI本身，至于内核模块、dtb、dtbo这些，说实话，老老实实用预编译的就行了。
