...
需要注意的是，WatchfacePackToolMi8只是一个编译器，并不能帮助你设计表盘，工程目录中的 `image` 文件夹包含了表盘界面的素材，`image_aod` 文件夹包含了表盘AOD界面的素材，而详细的配置则是在 `wfDef.json` 文件中，虽然json具有可读性，但不能像前面提到的工具那样实现“所见即所得”。
